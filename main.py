...
def main():
    nombre_jugador = input(
        "¡Bienvenido a Batallas Galácticas, una aventura en el espacio! Por favor, ingresa tu nombre: "
    )
    ...
    while enemigos:
        ...
        while enemigo_actual.salud > 0:
            ...
            if accion == "atacar":
                ...
                print(
                    f"Has atacado al {enemigo_actual.nombre} y le has causado {dano_jugador} puntos de daño."
                )
                enemigo_actual.recibir_dano(dano_jugador)
                if enemigo_actual.salud > 0:
                    dano_enemigo = enemigo_actual.atacar()
                    print(
                        f"El {enemigo_actual.nombre} te atacó y te causó {dano_enemigo} puntos de daño."
                    )
